...
**Nav dropdown accessibility.** The "Contacto" dropdown toggle (`.nav-dropdown-toggle`) has `aria-haspopup="menu"` and an `aria-expanded` state kept in sync with the existing CSS `:hover`/`:focus-within` show/hide via a small `mouseenter`/`mouseleave`/`focusin`/`focusout` listener block duplicated into each page's `<script>` — same copy-paste pattern as everything else. `BPP_Postulacion.html` has no dropdown (it uses a single `.nav-cta` link instead), so it's excluded from that snippet.
